...
// API polling — تحقق من طلبات جاهزة جديدة
if(isset($_GET['ajax']) && $_GET['ajax'] === 'poll') {
    ...
    // عدد الجاهز
    $ready_count = $pdo->prepare("SELECT COUNT(*) FROM orders WHERE restaurant_id=? AND status='ready'");
    $ready_count->execute([$rid]);

    echo json_encode([
        'new_ready'   => $new_ready,
        'new_orders'  => $new_orders,
        'ready_count' => $ready_count->fetchColumn(),
    ]);
    exit;
}
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id'])) {
    if(($_POST['status'] ?? '') === 'delivered') {
        $pdo->prepare("UPDATE orders SET status='delivered', staff_id=?, updated_at=NOW() WHERE id=? AND restaurant_id=?")
            ->execute([$_SESSION['staff_id'], $_POST['order_id'], $rid]);
