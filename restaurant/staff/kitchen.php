...
if(isset($_GET['ajax']) && $_GET['ajax'] === 'poll') {
    ...
    // نستخدم orders_v2 لو في branch_id، وإلا orders القديم
    if($branch_id) {
        $new = $pdo->prepare("
            SELECT o.id FROM orders_v2 o
        ...
        $counts_stmt = $pdo->prepare("
            SELECT status, COUNT(*) as c FROM orders_v2
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['order_id'])) {
        ...
        if(in_array($status, $allowed)) {
            ...
            // dual-write
            try {
                $pdo->prepare("UPDATE orders_v2 SET status=?, updated_at=NOW() WHERE legacy_order_id=? AND restaurant_id=?")
                    ->execute([$status, $_POST['order_id'], $rid]);
...
if($branch_id) {
    // نجيب من orders_v2 (مع legacy_order_id للـ items)
    $orders_stmt = $pdo->prepare("
        SELECT o2.*, o.id as legacy_id,
               COALESCE(o.restaurant_order_number, o2.id) as display_num
        FROM orders_v2 o2
        LEFT JOIN orders o ON o.id = o2.legacy_order_id
        WHERE o2.restaurant_id=? AND o2.branch_id=?
    ...
    // جلب الأصناف من legacy orders
    foreach($orders as &$ord) {
        $lid = $ord['legacy_id'] ?? $ord['id'];
        ...
        // نعدّل id ليشتغل مع الـ JS الموجود
        $ord['id'] = $ord['legacy_id'] ?? $ord['id'];
    }
    unset($ord);

    $max_id_stmt = $pdo->prepare("SELECT COALESCE(MAX(id),0) FROM orders_v2 WHERE restaurant_id=? AND branch_id=?");
...
body{font-family:'Tajawal',sans-serif;background:var(--bg);color:var(--ink);min-height:100vh;transition:background .3s,color .3s;}

/* TOPBAR */
.topbar{position:sticky;top:0;z-index:200;background:var(--bar);backdrop-filter:blur(20px);border-bottom:1px solid var(--line);padding:0 16px;height:56px;display:flex;align-items:center;gap:12px;}
.topbar-stripe{position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--p),transparent);}
.role-pill{display:flex;align-items:center;gap:6px;background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.22);color:#F59E0B;padding:4px 11px;border-radius:20px;font-size:11px;font-weight:800;flex-shrink:0;}
...
.icon-btn{width:36px;height:36px;border-radius:50%;background:var(--bg3);border:1px solid var(--line);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:background .2s,transform .3s;color:var(--ink2);text-decoration:none;font-size:13px;font-weight:700;font-family:'Tajawal',sans-serif;}
...
.logout-btn{padding:0 13px;border-radius:10px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.15);color:#EF4444;font-size:12px;width:auto;}

/* TABS */
.tabs-wrap{padding:12px 16px 0;background:var(--bg);position:sticky;top:56px;z-index:100;border-bottom:1px solid var(--line);}
.tabs{display:flex;gap:6px;}
.tab{display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:12px 12px 0 0;border:1px solid transparent;border-bottom:none;font-size:12px;font-weight:700;cursor:pointer;font-family:'Tajawal',sans-serif;color:var(--ink2);background:transparent;transition:all .2s;position:relative;bottom:-1px;}
.tab.active{background:var(--bg2);border-color:var(--line);color:var(--ink);}
.tab-badge{background:var(--p);color:#fff;padding:1px 7px;border-radius:10px;font-size:10px;font-weight:800;}
.tab-badge.amber{background:#F59E0B;}

/* SECTIONS */
.section{display:none;padding:14px 16px 80px;}
.section.active{display:block;}

/* STATUS BAR */
.status-bar{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:16px;}
.sbar-item{background:var(--bg2);border:1px solid var(--line);border-radius:14px;padding:12px;text-align:center;}
.sbar-num{font-family:'Fraunces',serif;font-size:22px;font-weight:900;line-height:1;}
.sbar-lbl{font-size:10px;font-weight:700;margin-top:3px;color:var(--ink2);}
.sbar-item.s-pending .sbar-num{color:#F59E0B;}
.sbar-item.s-confirmed .sbar-num{color:#3B82F6;}
.sbar-item.s-preparing .sbar-num{color:#8B5CF6;}
...
.ocard.new-flash{animation:newFlash .6s ease 3,cardIn .35s both;}
@keyframes newFlash{0%,100%{box-shadow:none;}50%{box-shadow:0 0 0 8px rgba(255,107,53,.2);}}
...
.ocard-num{font-family:'Fraunces',serif;font-size:16px;font-weight:900;color:var(--ink);}
.table-chip{background:rgba(255,107,53,.12);border:1px solid rgba(255,107,53,.22);color:var(--p);padding:3px 10px;border-radius:20px;font-size:11px;font-weight:800;}
.status-dot{width:8px;height:8px;border-radius:50%;margin-right:auto;flex-shrink:0;}
.sd-pending{background:#F59E0B;animation:pulse 1.5s infinite;}
.sd-confirmed{background:#3B82F6;}
.sd-preparing{background:#8B5CF6;animation:pulse 2s infinite;}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}
.ocard-elapsed{font-size:11px;font-weight:700;color:var(--ink2);}
.ocard-elapsed.warn{color:#EF4444;font-weight:900;}

/* Items */
.ocard-items{padding:10px 14px;flex:1;}
.oitem{
...
/* Options chips */
.oitem-opts{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:4px;}
.oitem-opt-chip{
...
.oitem-opt-chip::before{content:'◆';font-size:8px;}
...
.oitem-note{
    ...
    font-size:11px;color:#F59E0B;font-weight:600;margin-top:4px;
}
.oitem-note::before{content:'📝';font-size:11px;}
...
.ocard-note svg{width:12px;height:12px;flex-shrink:0;margin-top:1px;color:#F59E0B;}
...
.act-confirm{background:rgba(59,130,246,.1);color:#3B82F6;border-color:rgba(59,130,246,.2);}
.act-confirm:hover{background:#3B82F6;color:#fff;border-color:transparent;}
.act-prepare{background:rgba(139,92,246,.1);color:#8B5CF6;border-color:rgba(139,92,246,.2);}
.act-prepare:hover{background:#8B5CF6;color:#fff;border-color:transparent;}
.act-ready{background:rgba(34,197,94,.1);color:#22C55E;border-color:rgba(34,197,94,.2);}
.act-ready:hover{background:#22C55E;color:#fff;border-color:transparent;}
.act-cancel{background:rgba(239,68,68,.08);color:#EF4444;border-color:rgba(239,68,68,.15);}
.act-cancel:hover{background:#EF4444;color:#fff;border-color:transparent;}
...
/* SOLD OUT */
.dishes-list{display:flex;flex-direction:column;gap:7px;}
.dcard{background:var(--bg2);border:1px solid var(--line);border-radius:13px;padding:12px 14px;display:flex;align-items:center;gap:12px;}
.dcard.sold{border-color:rgba(245,158,11,.3);background:rgba(245,158,11,.04);}
.dcard-name{flex:1;font-size:13px;font-weight:600;color:var(--ink);}
.dcard.sold .dcard-name{color:#F59E0B;}
.sold-badge{font-size:10px;font-weight:800;color:#F59E0B;background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.2);padding:2px 8px;border-radius:10px;}
.sold-toggle{padding:7px 14px;border-radius:9px;font-size:12px;font-weight:800;cursor:pointer;font-family:'Tajawal',sans-serif;transition:all .2s;border:1.5px solid;flex-shrink:0;}
.sold-toggle.is-sold{background:rgba(34,197,94,.1);color:#22C55E;border-color:rgba(34,197,94,.2);}
.sold-toggle.is-sold:hover{background:#22C55E;color:#fff;border-color:transparent;}
.sold-toggle.not-sold{background:rgba(245,158,11,.1);color:#F59E0B;border-color:rgba(245,158,11,.2);}
.sold-toggle.not-sold:hover{background:#F59E0B;color:#fff;border-color:transparent;}
.section-head{font-family:'Fraunces',serif;font-size:17px;font-weight:900;margin-bottom:4px;}
.section-sub{font-size:12px;color:var(--ink2);margin-bottom:14px;}

/* TOAST */
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(20px);background:var(--bg2);border:1px solid var(--line);color:var(--ink);padding:10px 20px;border-radius:25px;font-size:13px;font-weight:700;z-index:999;opacity:0;transition:all .35s cubic-bezier(.34,1.56,.64,1);white-space:nowrap;backdrop-filter:blur(16px);pointer-events:none;}
...
<div class="tabs-wrap">
    <div class="tabs">
        ...
        <button class="tab" onclick="switchTab('soldout',this)">
            نفذ اليوم
            <?php $sold_count=array_sum(array_column($dishes,'sold_out')); if($sold_count>0): ?>
...
<div class="section active" id="section-orders">
    <div class="live-bar"><div class="live-dot"></div> يتجدد تلقائياً كل 8 ثوانٍ</div>
...
function testSound(){ userInteracted=true; playSound(); showToast('🔔 الصوت يعمل!'); }
function sendNotif(title,body){
    if(!notifGranted) return;
    try{ const n=new Notification(title,{body,tag:'kitchen',renotify:true}); n.onclick=()=>{window.focus();n.close();}; }catch(e){}
}
...
function toggleSoldOut(dishId,btn){
    ...
    .then(d=>{
        if(d.success){
            ...
            if(wasSold){ if(badge) badge.remove(); }
            else{ const b=document.createElement('span'); b.className='sold-badge'; b.textContent='نفذ'; card.insertBefore(b,btn); }
