...
if(isset($_GET['ajax']) && $_GET['ajax'] === 'poll') {
    ...
    if($branch_id) {
        $ready = $pdo->prepare("SELECT id, table_number FROM orders_v2 WHERE restaurant_id=? AND branch_id=? AND status='ready' AND id > ? ORDER BY id DESC LIMIT 5");
        $ready->execute([$rid, $branch_id, $last_ready_id]);
        $new = $pdo->prepare("SELECT id FROM orders_v2 WHERE restaurant_id=? AND branch_id=? AND id > ? AND status IN ('ready','preparing','confirmed','pending') LIMIT 3");
        $new->execute([$rid, $branch_id, $last_new_id]);
        $rc = $pdo->prepare("SELECT COUNT(*) FROM orders_v2 WHERE restaurant_id=? AND branch_id=? AND status='ready'");
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id'])) {
    if(($_POST['status'] ?? '') === 'delivered') {
        ...
        try {
            $pdo->prepare("UPDATE orders_v2 SET status='delivered', updated_at=NOW() WHERE legacy_order_id=? AND restaurant_id=?")
                ->execute([$_POST['order_id'], $rid]);
...
if($branch_id) {
    $orders_stmt = $pdo->prepare("
        ...
        WHERE o2.restaurant_id=? AND o2.branch_id=?
          AND o2.status IN ('ready','preparing','confirmed','pending')
        ORDER BY FIELD(o2.status,'ready','confirmed','preparing','pending'), o2.created_at ASC
    ");
