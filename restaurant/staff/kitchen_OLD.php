...
// API endpoint للـ polling
if(isset($_GET['ajax']) && $_GET['ajax'] === 'poll') {
    header('Content-Type: application/json');
    $last_id = intval($_GET['last_id'] ?? 0);

    $new = $pdo->prepare("SELECT o.id FROM orders o WHERE o.restaurant_id=? AND o.id > ? AND o.status IN ('pending','confirmed','preparing') LIMIT 5");
    $new->execute([$rid, $last_id]);
    $new_orders = $new->fetchAll();

    $counts = $pdo->prepare("SELECT status, COUNT(*) as c FROM orders WHERE restaurant_id=? AND status IN ('pending','confirmed','preparing') GROUP BY status");
    $counts->execute([$rid]);
    $status_counts = [];
    foreach($counts->fetchAll() as $r) $status_counts[$r['status']] = $r['c'];
...
$orders = $pdo->prepare("SELECT * FROM orders WHERE restaurant_id=? AND status IN ('pending','confirmed','preparing') ORDER BY created_at ASC");
$orders->execute([$rid]);
$orders = $orders->fetchAll();

// جلب أطباق كل طلب — مع options
foreach($orders as &$o) {
