...
if(isset($_SESSION['staff_id'])) {
    ...
    header('Location: ' . ($role === 'kitchen' ? 'kitchen.php' : 'waiter.php')); exit;
}
$error = '';
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    $username = trim($_POST['username']);
    $password = $_POST['password'];
    $stmt = $pdo->prepare("SELECT s.*, r.name as rest_name, r.slug FROM restaurant_staff s JOIN restaurants r ON r.id=s.restaurant_id WHERE s.username=? AND s.is_active=1");
    ...
    if($staff && password_verify($password, $staff['password'])) {
        ...
        $_SESSION['staff_number']     = $staff['staff_number'] ?? $staff['id'];
