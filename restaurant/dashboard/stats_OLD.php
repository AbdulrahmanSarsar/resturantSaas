...
$top_dishes = $pdo->prepare("
    ...
    WHERE o.restaurant_id = ? AND o.status = 'delivered' AND DATE(o.created_at) >= ?
...
");
$daily_orders->execute([$rid, $date_from]);
...
");
$top_cats->execute([$rid, $date_from]);
...
");
$prev->execute([$rid, $prev_from, $prev_to]);
$prev = $prev->fetch();
...
");
$offers_stats->execute([$rid, $date_from]);
...
$top_offers = $pdo->prepare("
    ...
    LIMIT 6
");
$top_offers->execute([$rid, $date_from]);
