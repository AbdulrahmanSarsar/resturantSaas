...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if($_POST['action'] === 'add') {
        ...
        if($name) {
            ...
            $pdo->prepare("INSERT INTO categories (restaurant_id, name, name_en, sort_order) VALUES (?,?,?,?)")
                ->execute([$rid, $name, $name_en, intval($_POST['sort_order'] ?? 0)]);
            $success = 'تم إضافة التصنيف بنجاح!';
        }
    }

    if($_POST['action'] === 'edit') {
        $name_en_edit = trim($_POST['name_en'] ?? '');
        $pdo->prepare("UPDATE categories SET name=?, name_en=?, sort_order=?, is_active=? WHERE id=? AND restaurant_id=?")
            ->execute([
                trim($_POST['name']),
                $name_en_edit,
    ...
    if($_POST['action'] === 'delete') {
        $pdo->prepare("DELETE FROM categories WHERE id=? AND restaurant_id=?")
            ->execute([intval($_POST['cat_id']), $rid]);
