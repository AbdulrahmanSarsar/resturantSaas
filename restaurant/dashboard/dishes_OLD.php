...
// جلب التصنيفات
$cats = $pdo->prepare("SELECT * FROM categories WHERE restaurant_id=? AND is_active=1 ORDER BY sort_order");
$cats->execute([$rid]);
$cats = $cats->fetchAll();
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if($_POST['action'] === 'add' || $_POST['action'] === 'edit') {
        ...
        // مسار مطلق بدل نسبي — يضمن الرفع الصحيح على أي سيرفر
        ...
        if(!empty($_FILES['image']['name']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if($ext === 'jfif') $ext = 'jpg'; // jfif = JPEG بامتداد مختلف
            if(in_array($ext, ['jpg','jpeg','png','webp','gif'])) {
                if($image && file_exists($upload_dir.$image)) @unlink($upload_dir.$image);
                $new_name = uniqid('dish_', true).'.'.$ext;
                if(move_uploaded_file($_FILES['image']['tmp_name'], $upload_dir.$new_name)) {
                    $image = $new_name;
                } else {
                    $success = '⚠️ فشل رفع الصورة — تحقق من صلاحيات مجلد assets/uploads/dishes/';
                }
            } else {
                $success = '⚠️ امتداد غير مدعوم: '.$ext;
            }
        }

        $model3d_file = $_POST['old_model'] ?? '';
        $usdz_file    = $_POST['old_usdz']  ?? '';
        $has_model3d  = 0;

        if(!empty($_FILES['model3d_usdz']['name'])) {
            $ext = strtolower(pathinfo($_FILES['model3d_usdz']['name'], PATHINFO_EXTENSION));
            if($ext === 'usdz') {
                if($usdz_file) @unlink('../../assets/uploads/models3d/'.$usdz_file);
                $usdz_file = uniqid().'.usdz';
                move_uploaded_file($_FILES['model3d_usdz']['tmp_name'], '../../assets/uploads/models3d/'.$usdz_file);
            }
        }
        if(!empty($_FILES['model3d']['name'])) {
            $ext = strtolower(pathinfo($_FILES['model3d']['name'], PATHINFO_EXTENSION));
            if($ext === 'glb') {
                if($model3d_file) @unlink('../../assets/uploads/models3d/'.$model3d_file);
                $model3d_file = uniqid().'.glb';
                move_uploaded_file($_FILES['model3d']['tmp_name'], '../../assets/uploads/models3d/'.$model3d_file);
                $has_model3d = 1;
            }
        } elseif($model3d_file) {
            $has_model3d = 1;
        ...
        if($_POST['action'] === 'add') {
            ...
            $ingredients_en = trim($_POST['ingredients_en'] ?? '');
            $recipe_en      = trim($_POST['recipe_en'] ?? '');
            $discount_percent = floatval($_POST['discount_percent'] ?? 0);
            $discount_active  = isset($_POST['discount_active']) ? 1 : 0;
            $prep_time = max(1, intval($_POST['prep_time'] ?? 15));
            $pdo->prepare("INSERT INTO dishes (restaurant_id,category_id,name,name_en,description,description_en,ingredients,ingredients_en,recipe,recipe_en,price,discount_percent,discount_active,prep_time,image,has_model3d,model3d_file,model3d_usdz,is_available,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
                ->execute([$rid,$category_id,$name,$name_en,$description,$description_en,$ingredients,$ingredients_en,$recipe,$recipe_en,$price,$discount_percent,$discount_active,$prep_time,$image,$has_model3d,$model3d_file,$usdz_file,$is_available,$sort_order]);
            $new_dish_id = $pdo->lastInsertId();

            // حفظ الاقتراحات اليدوية للطبق الجديد
            $pdo->prepare("DELETE FROM dish_suggestions WHERE dish_id=? AND restaurant_id=?")->execute([$new_dish_id,$rid]);
            $sugg_ids = $_POST['suggestions'] ?? [];
            foreach($sugg_ids as $ord => $sid) {
                $sid = intval($sid);
                if($sid && $sid !== $new_dish_id) {
                    $pdo->prepare("INSERT IGNORE INTO dish_suggestions (dish_id,suggested_dish_id,restaurant_id,sort_order) VALUES (?,?,?,?)")
                        ->execute([$new_dish_id,$sid,$rid,$ord]);
                }
            }
            $success = 'تم إضافة الطبق بنجاح!';
        } else {
            $did = intval($_POST['dish_id']);
            $name_en        = trim($_POST['name_en'] ?? '');
            $description_en = trim($_POST['description_en'] ?? '');
            $ingredients_en = trim($_POST['ingredients_en'] ?? '');
            $recipe_en      = trim($_POST['recipe_en'] ?? '');
            ...
            $discount_active  = isset($_POST['discount_active']) ? 1 : 0;
            $prep_time = max(1, intval($_POST['prep_time'] ?? 15));
            $pdo->prepare("UPDATE dishes SET category_id=?,name=?,name_en=?,description=?,description_en=?,ingredients=?,ingredients_en=?,recipe=?,recipe_en=?,price=?,discount_percent=?,discount_active=?,prep_time=?,image=?,has_model3d=?,model3d_file=?,model3d_usdz=?,is_available=?,sort_order=? WHERE id=? AND restaurant_id=?")
                ->execute([$category_id,$name,$name_en,$description,$description_en,$ingredients,$ingredients_en,$recipe,$recipe_en,$price,$discount_percent,$discount_active,$prep_time,$image,$has_model3d,$model3d_file,$usdz_file,$is_available,$sort_order,$did,$rid]);

            // تحديث الاقتراحات
            $pdo->prepare("DELETE FROM dish_suggestions WHERE dish_id=? AND restaurant_id=?")->execute([$did,$rid]);
            $sugg_ids = $_POST['suggestions'] ?? [];
            foreach($sugg_ids as $ord => $sid) {
                $sid = intval($sid);
                if($sid && $sid !== $did) {
                    $pdo->prepare("INSERT IGNORE INTO dish_suggestions (dish_id,suggested_dish_id,restaurant_id,sort_order) VALUES (?,?,?,?)")
                        ->execute([$did,$sid,$rid,$ord]);
                }
            }
            $success = 'تم تعديل الطبق!';
        }
    }

    if($_POST['action'] === 'delete') {
        $dish = $pdo->prepare("SELECT image,model3d_file FROM dishes WHERE id=? AND restaurant_id=?");
        $dish->execute([$_POST['dish_id'],$rid]);
        $dish = $dish->fetch();
        if($dish) {
            if($dish['image'])        @unlink('../../assets/uploads/dishes/'  .$dish['image']);
            if($dish['model3d_file']) @unlink('../../assets/uploads/models3d/'.$dish['model3d_file']);
        }
        $pdo->prepare("DELETE FROM dishes WHERE id=? AND restaurant_id=?")->execute([$_POST['dish_id'],$rid]);
        $success = 'تم حذف الطبق!';
    }

    if($_POST['action'] === 'toggle') {
        $pdo->prepare("UPDATE dishes SET is_available=NOT is_available WHERE id=? AND restaurant_id=?")
            ->execute([$_POST['dish_id'],$rid]);
        if(isset($_POST['ajax'])) { echo json_encode(['success'=>true]); exit; }
    }

    if($_POST['action'] === 'reset_soldout') {
        $pdo->prepare("UPDATE dishes SET sold_out=0 WHERE restaurant_id=?")->execute([$rid]);
    ...
    if($_POST['action'] === 'save_options') {
        $did = intval($_POST['dish_id']);
        $chk = $pdo->prepare("SELECT id FROM dishes WHERE id=? AND restaurant_id=?");
        $chk->execute([$did, $rid]);
        if($chk->fetch()) {
            $pdo->prepare("DELETE FROM dish_option_values WHERE group_id IN (SELECT id FROM dish_option_groups WHERE dish_id=? AND restaurant_id=?)")
                ->execute([$did, $rid]);
            $pdo->prepare("DELETE FROM dish_option_groups WHERE dish_id=? AND restaurant_id=?")
                ->execute([$did, $rid]);
            $groups = $_POST['opt_groups'] ?? [];
            foreach($groups as $gidx => $group) {
                $gname    = trim($group['name'] ?? '');
                $gname_en = trim($group['name_en'] ?? '');
                ...
                if(!$gname) continue;
                $pdo->prepare("INSERT INTO dish_option_groups (dish_id,restaurant_id,name,name_en,price_mode,is_required,sort_order) VALUES (?,?,?,?,?,?,?)")
                    ->execute([$did, $rid, $gname, $gname_en, $gmode, $greq, $gsort]);
                $gid = $pdo->lastInsertId();
                $values = $group['values'] ?? [];
                foreach($values as $vidx => $val) {
                    $vname    = trim($val['name'] ?? '');
                    $vname_en = trim($val['name_en'] ?? '');
                    $vprice   = floatval($val['price'] ?? 0);
                    $vsort    = intval($val['sort'] ?? $vidx);
                    if(!$vname) continue;
                    $pdo->prepare("INSERT INTO dish_option_values (group_id,name,name_en,price,sort_order) VALUES (?,?,?,?,?)")
                        ->execute([$gid, $vname, $vname_en, $vprice, $vsort]);
                }
            }
        }
...
if($filter_cat) {
    $dishes = $pdo->prepare("SELECT d.*,c.name as cat_name FROM dishes d LEFT JOIN categories c ON c.id=d.category_id WHERE d.restaurant_id=? AND d.category_id=? ORDER BY d.sort_order,d.id DESC");
    $dishes->execute([$rid,$filter_cat]);
} else {
    $dishes = $pdo->prepare("SELECT d.*,c.name as cat_name FROM dishes d LEFT JOIN categories c ON c.id=d.category_id WHERE d.restaurant_id=? ORDER BY d.sort_order,d.id DESC");
    $dishes->execute([$rid]);
}
...
// اقتراحات كل طبق (للتعبئة عند الفتح)
$sugg_map = [];
$sugg_rows = $pdo->prepare("SELECT dish_id,suggested_dish_id FROM dish_suggestions WHERE restaurant_id=? ORDER BY sort_order");
$sugg_rows->execute([$rid]);
foreach($sugg_rows->fetchAll() as $sr) {
    $sugg_map[$sr['dish_id']][] = $sr['suggested_dish_id'];
}

// خيارات كل طبق
$options_map = [];
$og_rows = $pdo->prepare("SELECT * FROM dish_option_groups WHERE restaurant_id=? ORDER BY sort_order");
$og_rows->execute([$rid]);
foreach($og_rows->fetchAll() as $og) {
    $ov_rows = $pdo->prepare("SELECT * FROM dish_option_values WHERE group_id=? ORDER BY sort_order");
    $ov_rows->execute([$og['id']]);
    $og['values'] = $ov_rows->fetchAll();
    $options_map[$og['dish_id']][] = $og;
}
