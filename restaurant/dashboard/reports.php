...
");
$today->execute(array_merge([$rid], $bp));
$td = $today->fetch();
...
$daily = $pdo->prepare("
    SELECT 
        ...
        COALESCE(SUM(total_price),0) as revenue
    FROM orders
    WHERE restaurant_id=? $bf AND payment_status='paid' AND created_at >= DATE_SUB(CURDATE(),INTERVAL 29 DAY)
    GROUP BY DATE(created_at)
    ORDER BY day ASC
");
$daily->execute(array_merge([$rid], $bp));
...
$monthly = $pdo->prepare("
    ...
    ORDER BY month ASC
");
$monthly->execute(array_merge([$rid], $bp));
...
$top_dishes = $pdo->prepare("
    ...
    WHERE o.restaurant_id=? $bfo AND o.payment_status='paid' AND DATE(o.created_at)>=DATE_SUB(CURDATE(),INTERVAL 30 DAY)
    GROUP BY oi.dish_name ORDER BY qty DESC LIMIT 5
");
$top_dishes->execute(array_merge([$rid], $bp));
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px;flex-wrap:wrap;gap:12px;">
    ...
    <a href="branch_comparison.php" class="btn btn-primary btn-sm" style="text-decoration:none;">
...
<div class="report-tabs">
    ...
    <a href="branch_comparison.php"   class="rtab">🏠 الفروع</a>
</div>
