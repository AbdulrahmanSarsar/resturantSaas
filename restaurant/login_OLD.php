...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM restaurants WHERE email=? AND is_active=1");
    $stmt->execute([$email]);
    ...
    if($restaurant && password_verify($password, $restaurant['password'])) {
        if($restaurant['subscription_expiry'] && $restaurant['subscription_expiry'] < date('Y-m-d')) {
            ...
            $_SESSION['restaurant_plan'] = $restaurant['subscription_plan'];
...
.hero-heading {
    ...
    animation: fadeUp 0.7s cubic-bezier(0.22,1,0.36,1) .18s both;
}
.hero-heading em {
    font-style: italic; color: var(--p); font-weight: 300;
}
.hero-sub {
    font-size: 15px; color: var(--ink2); line-height: 1.75;
    max-width: 380px;
    animation: fadeUp 0.7s cubic-bezier(0.22,1,0.36,1) .26s both;
}

/* Feature pills */
.pills {
    display: flex; flex-wrap: wrap; gap: 9px; margin-top: 32px;
    animation: fadeUp 0.7s cubic-bezier(0.22,1,0.36,1) .32s both;
}
.pill {
...
.stats-row {
    ...
    animation: fadeUp 0.7s cubic-bezier(0.22,1,0.36,1) .38s both;
}
.stat-item {}
.stat-num {
    font-family: 'Fraunces', serif;
...
.deco-number {
    ...
    font-family: 'Fraunces', serif; font-size: 220px; font-weight: 900;
    color: rgba(255,107,53,0.04); line-height: 1;
...
.login-heading {
    margin-bottom: 36px;
    animation: fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) .05s both;
}
.login-heading-top {
...
.fg:nth-child(2){animation-delay:.18s}

.fg label {
    display: block; font-size: 11px; font-weight: 700;
    color: var(--ink3); letter-spacing: 1.5px; text-transform: uppercase;
    margin-bottom: 9px;
}
...
.submit-btn {
    ...
    animation: fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) .28s both;
}
...
.back-link {
    ...
    animation: fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) .38s both;
}
...
<div class="left-panel">
    ...
    <div class="hero-area">
        ...
        <div class="hero-sub">
            حوّل قائمة طعامك إلى تجربة رقمية غامرة مع QR، عرض AR ثلاثي الأبعاد، وإدارة طلبات في الوقت الحقيقي.
        </div>
    ...
    <div class="stats-row">
        <div class="stat-item">
            ...
            <div class="stat-lbl">مطعم نشط</div>
        </div>
        <div class="stat-item">
            <div class="stat-num">3<span>باقات</span></div>
            <div class="stat-lbl">لكل احتياج</div>
        </div>
        <div class="stat-item">
    ...
    <div class="form-wrap">
        ...
        <div class="login-heading">
            <div class="login-heading-top">
                <span class="login-badge">RESTAURANT</span>
            </div>
            <div class="login-title-text">
                مرحباً<br><em>بعودتك</em>
            </div>
        ...
        <form method="POST" id="loginForm">
            ...
            <div class="fg">
                ...
                <div class="input-wrap">
                    ...
                    <button type="button" class="pass-eye" onclick="togglePass()" id="eyeBtn">
                        <svg id="eyeShow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                            <circle cx="12" cy="12" r="3"/>
                        </svg>
                        <svg id="eyeHide" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="display:none">
