...
<body>
    ...
    <div class="content">
        <div class="empty-cart" id="emptyState">
            ...
            <a href="<?= BASE_URL ?>/menu/<?= $slug ?>/<?= $branch_url ?>" class="browse-btn" id="browseBtn">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
                    <path d="M3 2l3 7H6a3 3 0 003 3v9a1 1 0 001 1h4a1 1 0 001-1v-9a3 3 0 003-3h-3l3-7" />
                </svg>
                تصفح المنيو
            </a>
        </div>
    ...
    <script>
        ...
        function t(k) {
            return TR[currentLang][k] || TR.ar[k] || k;
        }
        ...
        function getSubtotal() {
            return cart.reduce((s, i) => s + (parseFloat(i.price) || 0) * (parseInt(i.qty) || 0), 0);
        }
        ...
        function updateSummary() {
            ...
            if (disc > 0) {
                ...
                document.getElementById('discountVal').textContent = '-' + fmtPrice(disc);
        ...
        function applyCoupon() {
            ...
            if (!code) {
                ...
                msg.textContent = '⚠️ ' + (currentLang === 'en' ? 'Enter coupon code first' : 'أدخل كود الكوبون أولاً');
                return;
            }
            ...
            fetch(`${BASE_URL}/menu/get_suggestions.php`, {
                ...
                .then(data => {
                    if (data.suggestions && data.suggestions.length) {
                        ...
                        setTimeout(() => {
                            document.getElementById('suggOverlay').classList.add('open');
                            sessionStorage.setItem(SUGG_KEY, '1');
                        }, 700);
                    }
                })
                .catch(() => {});
        ...
        function closeSugg() {
            document.getElementById('suggOverlay').classList.remove('open');
        }
        document.getElementById('suggOverlay').addEventListener('click', function(e) {
            if (e.target === this) closeSugg();
        });
