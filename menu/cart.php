 ...
 *   - dual-write: orders + orders_v2 (مع branch_id)
...
// لو ما في فرع مرتبط → null (ما نحشر $rid بـ orders_v2)
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['items'])) {
    ...
    if(empty($items) || !$table) {
        ...
        // ===== Legacy write: orders =====
        // كتابة branch_id بالجدول القديم — للفصل بين الفروع
        try {
            ...
            // العمود branch_id مش موجود بعد — استخدم INSERT بدونه
        ...
        // ===== DUAL WRITE: orders_v2 — بس لو عندنا branch_id صالح =====
        try {
            // لو ما في branch_id → ما نكتب بـ orders_v2 (foreign key constraint)
            if(!$branch_id) throw new \Exception('no_branch_skip');

            $orgStmt = $pdo->prepare("SELECT rv.organization_id FROM restaurants_v2 rv WHERE rv.id = ?");
            $orgStmt->execute([$rid]);
            $orgRow = $orgStmt->fetch();
            $org_id = $orgRow ? $orgRow['organization_id'] : $rid;

            $pdo->prepare("INSERT INTO orders_v2 (
                ...
                tax_details, coupon_code, status, legacy_order_id
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,'pending',?)")
            ->execute([
                $branch_id, $rid, $org_id,
                $table, $customer, $notes,
                round($total, 2), round($discount_amt, 2), round($tax_amount, 2), $grand_total,
                json_encode($tax_details), $coupon_code ?: null, $order_id,
            ]);
            $new_order_id = $pdo->lastInsertId();

            $oldItems = $pdo->prepare("SELECT * FROM order_items WHERE order_id = ?");
            $oldItems->execute([$order_id]);
            foreach($oldItems->fetchAll() as $oi) {
                $pdo->prepare("INSERT INTO order_items_v2 (order_id,dish_id,dish_name,dish_name_en,dish_price,quantity,notes,options) VALUES (?,?,?,?,?,?,?,?)")
                    ->execute([$new_order_id,$oi['dish_id'],$oi['dish_name'],$oi['dish_name_en'],$oi['dish_price'],$oi['quantity'],$oi['notes'],$oi['options']]);
            }

            $pdo->prepare("INSERT INTO order_status_log_v2 (order_id,status,changed_at) VALUES (?,'pending',NOW())")
                ->execute([$new_order_id]);

        } catch(\Exception $e) {
            if($e->getMessage() !== 'no_branch_skip') {
                error_log('cart dual-write failed: ' . $e->getMessage());
            }
        }
