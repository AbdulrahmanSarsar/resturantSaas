...
$cur_prefix    = in_array($cur_symbol,    ['$', "â¬", "âº"]);
$cur_prefix_en = in_array($cur_symbol_en, ['$', "â¬", "âº"]);
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['items'])) {
    ...
    if(empty($items) || !$table) {
        ...
        foreach($active_taxes as $atax) {
            ...
            $tax_amount += $tamt;
            $tax_details[] = ['name'=>$atax['name'],'name_en'=>$atax['name_en']??'','type'=>$atax['type'],'value'=>$atax['value'],'amount'=>round($tamt,2)];
        }
        $grand_total = $final_total + $tax_amount;

        $pdo->prepare("INSERT INTO orders (restaurant_id,table_number,customer_name,customer_notes,total_price,coupon_code,discount_amount,tax_amount,tax_details,status) VALUES (?,?,?,?,?,?,?,?,?,'pending')")
            ->execute([$rid,$table,$customer,$notes,$grand_total,$coupon_code?:null,$discount_amt,round($tax_amount,2),json_encode($tax_details)]);
        ...
        foreach($items as $item) {
            ...
            // طبق عادي
            $dish = $pdo->prepare("SELECT * FROM dishes WHERE id=? AND restaurant_id=?");
            $dish->execute([intval($item['id']), $rid]);
            $dish = $dish->fetch();
            if(!$dish) continue;
            // احسب السعر الفعلي (المخفض لو فيه خصم نشط)
            $dp   = floatval($dish['discount_percent'] ?? 0);
            $da   = !empty($dish['discount_active']);
            $real_price = ($da && $dp > 0) ? $dish['price'] * (1 - $dp/100) : $dish['price'];
            // استخدم السعر اللي أرسله الزبون لو أقل من السعر الأصلي (حماية)
            $item_price = isset($item['price']) ? floatval($item['price']) : $real_price;
            // لو السعر المرسل أكبر من السعر الأصلي → استخدم السعر الأصلي
            if($item_price > $dish['price']) $item_price = $dish['price'];
            $pdo->prepare("INSERT INTO order_items (order_id,dish_id,dish_name,dish_name_en,dish_price,quantity,notes,options) VALUES (?,?,?,?,?,?,?,?)")
                ->execute([$order_id,$dish['id'],$dish['name'],$dish['name_en']??'',$item_price,$item_qty,$item_notes,$item_options]);
        }
        if($coupon_code) {
            $pdo->prepare("UPDATE coupons SET used_count=used_count+1 WHERE code=? AND restaurant_id=?")
                ->execute([$coupon_code,$rid]);
        }
        header("Location: ".BASE_URL."/menu/$slug/order/$order_id"); exit;
    }
}
...
.header-title{font-family:'Fraunces',serif;font-size:18px;font-weight:700;color:var(--ink);flex:1;letter-spacing:-0.3px;}
...
.summary-label{color:var(--ink2);font-weight:500;}
...
.order-btn:disabled::after{display:none;}
...
.sugg-title{font-family:'Fraunces',serif;font-size:16px;font-weight:900;color:var(--ink);letter-spacing:-0.3px;}
...
<div class="header">
    <a href="<?= BASE_URL ?>/menu/<?= $slug ?>" class="back-btn">
...
<div class="content">
    <div class="empty-cart" id="emptyState">
        ...
        <a href="<?= BASE_URL ?>/menu/<?= $slug ?>" class="browse-btn" id="browseBtn">
...
const TR = {
    ar: {
        ...
        lbl_items:     'الأصناف المختارة',
        lbl_details:   'بيانات الطلب',
        lbl_coupon:    'كوبون الخصم',
        lbl_summary:   'ملخص الطلب',
        lbl_subtotal:  'المجموع الفرعي',
        lbl_discount:  'خصم الكوبون',
        lbl_total:     'الإجمالي',
        table_ph:      'رقم الطاولة *',
        table_req:     '⚠️ رقم الطاولة مطلوب',
        name_ph:       'اسمك (اختياري)',
        notes_ph:      'ملاحظات إضافية...',
        coupon_ph:     'كود الخصم...',
        apply:         'تطبيق',
        confirm_order: 'تأكيد الطلب',
        sending:       'جاري إرسال الطلب...',
        qr_auto:       'تم تحديده تلقائياً من QR Code',
        add:           'أضف للسلة',
        added:         'تمت الإضافة!',
        no_thanks:     'لا شكراً، السلة كافية',
        you_may_like:  'ممكن يعجبك كمان',
        others_ordered:'زباين طلبوا هالأطباق مع طلبك',
        dir:           'rtl', lang: 'ar',
    },
    en: {
        ...
        lbl_items:     'Selected Items',
        lbl_details:   'Order Details',
        lbl_coupon:    'Discount Coupon',
        lbl_summary:   'Order Summary',
        lbl_subtotal:  'Subtotal',
        lbl_discount:  'Coupon Discount',
        lbl_total:     'Total',
        table_ph:      'Table Number *',
        table_req:     '⚠️ Table number is required',
        ...
        qr_auto:       'Auto-detected from QR Code',
        add:           'Add',
        added:         'Added!',
        no_thanks:     'No thanks',
        you_may_like:  'You Might Also Like',
        others_ordered:'Customers ordered these with similar orders',
        dir:           'ltr', lang: 'en',
    }
};
